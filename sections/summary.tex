...
\begin{itemize}
	...
	\end{itemize}
\end{itemize}
\todo[inline]{Update with final questions}
...
Each article includes three types of metadata that we chose to extend our topic models with: author, category, and taxonomy metadata.
...
Instead, each of $A$ authors and $C$ categories has its own topic distribution.
...
For the author-topic and category-topic model, we we also able to analyze the similarity between author pairs and category pairs.
...
For the taxonomy-topic model, it was found that the topics in general were the most understandable of all topic model, which made sense with it having the highest topic coherence.
...
The analysis shows that each model has its own usefulness in supporting recommendation of articles depending on what kind of recommendation is wanted.

%appendix
As further experimentation and exploration, we tested what would happen if we did further preprocessing, if we included multiple topic distributions in our models, and if we used the author and category metadata in the \gls{pam}.
...
Though, while this is the case, having two topic distributions will give more possibilities for recommendation of articles compared to having one topic distribution.

We also made two other models called the Author-Doc and Category-Doc model.
