...
Our standard \gls{lda} model uses a similar notation to \citet{blei2003latent}, where a document $d$ is a vector of $N$ ID's, where the ID represents the given word in the original document. $Z_{d,n}$ is the topic assigned to a word $n$ within $d$.\todo[inline]{In the original LDA paper, a document is actually a sequence of N words, where each word is a one-hot vector encoding. Might not be important to change, since we just say the notation is 'similar'.}
...
\todo[inline]{We might need to figure out a way to easily differentiate in text between our own author lda model and the author-topic model from Rozen-Zvi.}
