...
It setup in a tree structure with the first layer being the root node, a topic which all documents are part of, and the bottom layer consisting of leaf nodes which are words. 

For simplicity we will be using five-level pachinko tree structure, with each node in a layer having an edge to each node in the next layer.
We also therefore limit the taxonomy chain to the first $3$ taxonomy entries, in order to fit with our DAG-structure.
...
\begin{figure}
	\centering
	\includegraphics[width= 0.9 \linewidth]{figures/five_layer_pachinko.png}
	\caption{Plate notation for five-level PAM}
	\label{fig:pachinko_plate}
\end{figure}
\todo[inline]{make pretty picture for pachinko plate notation}

\subsubsection{Modification to Pachinko Allocation}
Since only $25\%$ of the documents have an observed taxonomy, we can then lock the words in the observed documents to be in the corresponding topics within the pachinko DAG instead of conteniously sampling them using gibbs sampling.
