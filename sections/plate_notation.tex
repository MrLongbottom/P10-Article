...
\begin{equation}\label{eq:pachinko_gibbs}
	...
	\end{split}
\end{equation}
As in \citet{li2006pachinko} $t_1$ is the root topic and $Z_{w2}$, $Z_{w3}$, $Z_{w4}$ are topic assignments for the three middle layers of topics in our 5-layer Pachinko model.
...
In the combination model(Author-Category), we simply multiply the topic distributions $\theta_a$ and $\theta_c$ together, when sampling for a new topic.
