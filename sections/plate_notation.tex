...
\begin{equation}\label{eq:pachinko_gibbs}
	...
	\end{split}
\end{equation}
As in \citet{li2006pachinko} $t_1$ is the root topic and $Z_{w2}$, $Z_{w3}$, $Z_{w4}$ are the three middle layers of topics in our 5-layer Pachinko model.
$Z_{-w}$ is the word topic assignment, for all other words than the one we are working on.
Without looking at the current object of iteration, $n_x^d$ is the number of times topic $tx$ occurs in document $d$. 
...
The idea behind using five layers instead of four is because, the taxonomy field contains up to 5 layers of information. 
