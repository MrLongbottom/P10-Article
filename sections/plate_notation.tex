...
We have created a combination model to see whether using multiple meta-data fields at the same time to draw topics, would have an effect on the performance of the topic model.
The first model, we have created is the Author-Category combination model.
To combine the Author and Category metadata, we simply multiply the topic distributions $\theta_a$ and $\theta_c$, when drawing a new topic.
Then, when sampling for a new word topic, we normalize the product of the multiplication to sum to 1, which also makes it a distribution. \vejleder[inline]{mathify this}
