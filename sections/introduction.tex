...
Broadcasting information around the world is a crucial skill, that is used every day, as news articles, television and newspapers supply people with the latest information.
Searching and categorizing the news is becoming a bigger problem, since news are created every day, if not every hour.
As the internet, becomes more personalized, like targeted advertisement, recommendation is seeing more use in applications, such as Netflix and Amazon.
Within recommendation, there are two main machine learning techniques: Collaborative filtering and Content based filtering.
These two techniques are based on assumption that similar users like similar items (collaborative) or one user likes certain types items(content based).
The recommendation task can be very specific depending on the dataset that is used.
We focus on creating a better environment for recommendation of news articles.

One particular field, which covers a variety of different use cases and application is the field of topic modeling.
Topic modeling has been used for finding topics within a collection of document, to either annotate or categories certain collections of texts\cite{red_article_blei}.   
