\documentclass[conference, dvipsnames]{IEEEtran}

% Usepackages
\input{setup/preamble.tex}

% Macros
\input{setup/macros.tex}

% Glossaries
\input{setup/acronyms.tex}


\begin{document}
% Autoref
\def\sectionautorefname{Section}
\def\subsectionautorefname{Section}
\def\subsubsectionautorefname{Section}
%\def\figureautorefname{Fig.}
\def\definitionautorefname{Definition}

\title{Integrating News Metadata into Topic Models\\
%\thanks{Identify applicable funding agency here. If none, delete this.}
}

%\IEEEauthorblockN{Dennis Højbjerg Rose, Peter Langballe Erichsen, and Rasmus Engesgaard Christensen}\\
%\IEEEauthorblockA{Department of Computer Science, Aalborg University,\\Selma Lagerløfs Vej 300, 9220 Aalborg Øst, Denmark\\Email: \{drose16, perich16, rech16\}@student.aau.dk}}

\author{
\IEEEauthorblockN{Dennis Højbjerg Rose}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{Aalborg University}\\
Aalborg, Denmark \\
drose16@student.aau.dk}
\and
\IEEEauthorblockN{Peter Langballe Erichsen}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{Aalborg University}\\
Aalborg, Denmark \\
perich16@student.aau.dk}
\and
\IEEEauthorblockN{Rasmus Engesgaard Christensen}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{Aalborg University}\\
Aalborg, Denmark \\
rech16@student.aau.dk}
}

\maketitle

\begin{abstract}
	\input{sections/Abstract.tex}
\end{abstract}

\glsresetall

\begin{IEEEkeywords}
	component, formatting, style, styling, insert
\end{IEEEkeywords}
\todo[inline]{Find keywords. IEEE approved keywords can be found at: \url{https://ieeecs-media.computer.org/assets/pdf/taxonomy.pdf}}
\glsresetall

\todo[inline]{reference the appendix more in the paper}
% - Introduction (7.5 \%) 
\input{sections/introduction.tex}
%
%% - Related works (7.5 \%))
\input{sections/related_works.tex}
%
%% - Preliminaries/"Experiment" (20 \%)
%%		- Data
%%		- Setup
%%		- Analysis


\input{sections/Dataset.tex}
\input{sections/plate_notation.tex}

%
%\input{sections/method/preprocessing}
%\input{sections/method/method.tex}
%\input{sections/method/query_handling.tex}

%\input{sections/Dataset.tex}
%%\input{sections/Pipeline.tex}
\input{sections/Experiment.tex}
%% - Body(50 \%)
%%	- Results 
%
%%	- Discussion
%%		- Main results
%%		- Other results
%%		- Error sources
\input{sections/Discussion.tex}
%
%
%% - Conclusion (5 \%)
%%	- (Possible) Future work
\input{sections/conclusion}
\input{sections/futureWork}
%
%\input{sections/Acknowledgements}

% - References (10 \%)
% 	- Acknowledgments
%	- Bibliography
\bibliography{paper}

%Appendixes, if needed, appear before the acknowledgment.
\glsresetall
\onecolumn
\appendix
% Intro
\input{sections/appendix/Intro.tex}

% Overview
\input{sections/appendix/Overview.tex}
\input{sections/appendix/metadata_labels.tex}
\input{sections/appendix/metrics.tex}
%\input{sections/appendix/perplexity.tex}
\input{sections/appendix/grid_search.tex}
\input{sections/appendix/stemming.tex}
\input{sections/appendix/author_category.tex}
\input{sections/appendix/color_articles.tex}
% Code
\input{sections/appendix/gibbs_sampling.tex}
\input{sections/appendix/multi_process_gibbs.tex}
\input{sections/appendix/pachinko_implementation.tex}
\input{sections/appendix/locking.tex}
\input{sections/appendix/pachinko_results.tex}
\input{sections/appendix/cat_auth_pachinko.tex}
\input{sections/appendix/models.tex}
\input{sections/appendix/pyro.tex}

% Applications
\input{sections/appendix/applications.tex}

\end{document}
