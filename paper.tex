\documentclass[conference]{IEEEtran}
%\IEEEoverridecommandlockouts
% The preceding line is only needed to identify funding in the first footnote. If that is unneeded, please comment it out.
% Usepackages
\input{setup/preamble.tex}

% Macros
\input{setup/macros.tex}

% Glossaries
\input{setup/acronyms.tex}


\begin{document}

\title{Very Incredibly Complex Title\\
%\thanks{Identify applicable funding agency here. If none, delete this.}
}

%\IEEEauthorblockN{Dennis Højbjerg Rose, Peter Langballe Erichsen, and Rasmus Engesgaard Christensen}\\
%\IEEEauthorblockA{Department of Computer Science, Aalborg University,\\Selma Lagerløfs Vej 300, 9220 Aalborg Øst, Denmark\\Email: \{drose16, perich16, rech16\}@student.aau.dk}}

\author{
\IEEEauthorblockN{1\textsuperscript{st} Dennis Højbjerg Rose}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{Aalborg University}\\
Aalborg, Denmark \\
drose16@student.aau.dk}
\and
\IEEEauthorblockN{2\textsuperscript{nd} Peter Langballe Erichsen}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{Aalborg University}\\
Aalborg, Denmark \\
perich16@student.aau.dk}
\and
\IEEEauthorblockN{3\textsuperscript{rd} Rasmus Engesgaard Christensen}
\IEEEauthorblockA{\textit{Department of Computer Science} \\
\textit{Aalborg University}\\
Aalborg, Denmark \\
rech16@student.aau.dk}
}

\maketitle

\begin{abstract}
	\input{sections/Abstract.tex}
\end{abstract}

\glsresetall

\begin{IEEEkeywords}
	component, formatting, style, styling, insert
\end{IEEEkeywords}

\glsresetall

% - Introduction (7.5 \%) 
\input{sections/introduction.tex}
%
%% - Related works (7.5 \%))
\input{sections/related_works.tex}
%
%% - Preliminaries/"Experiment" (20 \%)
%%		- Data
%%		- Setup
%%		- Analysis

\input{sections/Overview.tex}
\input{sections/Dataset.tex}
\input{sections/plate_notation.tex}

%
%\input{sections/method/preprocessing}
%\input{sections/method/method.tex}
%\input{sections/method/query_handling.tex}

%\input{sections/Dataset.tex}
%%\input{sections/Pipeline.tex}
\input{sections/Experiment.tex}
%% - Body(50 \%)
%%	- Results 
%
%%	- Discussion
%%		- Main results
%%		- Other results
%%		- Error sources
\input{sections/Discussion.tex}
%
%
%% - Conclusion (5 \%)
%%	- (Possible) Future work
\input{sections/conclusion}
\input{sections/futureWork}
%
%\input{sections/Acknowledgements}

% - References (10 \%)
% 	- Acknowledgments
%	- Bibliography

\bibliography{paper}

%Appendixes, if needed, appear before the acknowledgment.
\appendix
\subsection{Metadata}
\todo[inline]{Update numbers in metadata sections when preprocessing is completely finished.}
\input{sections/appendix/categories.tex}
\input{sections/appendix/authors.tex}
%\input{sections/appendix/query_expansion.tex}
%\input{sections/appendix/variational_inference.tex}
%\input{sections/appendix/problems.tex}
%\input{sections/appendix/search_result.tex}
%\input{sections/appendix/perplexity_granularity.tex}
%\input{sections/appendix/word_count_distributions.tex}
%\input{sections/appendix/topic_documents.tex}

% Examples of tables and figure
%\input{examples.tex}

%\section{Unused Sections}
%\input{unusedSections/KnowledgeGraph.tex}
\end{document}
